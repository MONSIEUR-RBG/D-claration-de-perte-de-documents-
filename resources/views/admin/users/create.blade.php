...
<head>
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; font-family:'Nunito',sans-serif; }
        ...
        .alert-danger { background:#f8d7da; color:#721c24; padding:1rem 1.5rem; border-radius:12px; margin-bottom:1.5rem; border-left:4px solid #e74c3c; }
    </style>
</head>
<body>
...
<div class="main-content">
    ...
    <div class="container">
        @if($errors->any())
            <div class="alert-danger"><ul style="margin-left:1.5rem;">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
        @endif
        <div class="card" style="background:white;border-radius:20px;padding:2rem;box-shadow:0 10px 30px rgba(0,0,0,0.05);border:1px solid rgba(102,126,234,0.1);">
            <form method="POST" action="{{ route('admin.users.store') }}">@csrf
                <div class="form-group"><label class="form-label">Nom complet *</label><input type="text" name="name" class="form-control" value="{{ old('name') }}" required autofocus></div>
